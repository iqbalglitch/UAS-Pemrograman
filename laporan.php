	...
	die();
	 ...
	 echo '
		...
		<div class="col-sm-6"><table class="table table-bordered">';
		 ...
		 $sql = mysqli_query($koneksi, "SELECT count(nama), sum(biaya) FROM transaksi WHERE tanggal BETWEEN '$tgl1' AND '$tgl2'");

		 list($nama, $biaya) = mysqli_fetch_array($sql);{
			echo '<tr><td><span class="pull-right"><h4><b>'.$nama.' Orang</b></h4></span></td><td><span class="pull-right"><h4><b>RP. '.number_format($biaya).'</b></h4></span></td></tr>';
...
<?php
	  ...
	  $sql = mysqli_query($koneksi, "SELECT count(nama), sum(biaya) FROM transaksi WHERE tanggal='$tanggal'");
