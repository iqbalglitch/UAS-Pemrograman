...
if( empty( $_SESSION['id_user'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		$jenis = $_REQUEST['makan'];
		$biaya = $_REQUEST['biaya'];

		$sql = mysqli_query($koneksi, "UPDATE biaya SET jenis='$jenis', makan='$makan', biaya='$biaya' WHERE id_biaya='$id_biaya'");
...
<form method="post" action="" class="form-horizontal" role="form">
	...
	<div class="form-group">
		<label for="makan" class="col-sm-2 control-label">Nama Makanan</label>
		<div class="col-sm-3">
			<input type="text" class="form-control" id="makan" value="<?php echo $row['makan']; ?>" name="makan" placeholder="Nama Makanan" required>
		</div>
	</div>
	<div class="form-group">
